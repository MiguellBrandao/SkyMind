...
import { ALL_TOOLS, TOOLS_BY_NAME } from "../tools/registry";
...
async function executeTool(call: ToolCall, ctx: ToolContext): Promise<unknown> {
...
const TOOL_DEFINITIONS: ToolDefinition[] = ALL_TOOLS.map((tool) => ({
  name: tool.name,
...
export async function runAgent(options: AgentRunOptions): Promise<AgentRunResult> {
  ...
  for (let iteration = 0; iteration < MAX_TOOL_ITERATIONS; iteration++) {
    ...
    for (const call of result.toolCalls) {
      ...
      const output = await executeTool(call, options.toolContext);

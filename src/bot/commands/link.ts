...
export const linkCommand: SlashCommand = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    try {
      const result = await linkService.startLink(interaction.user.id, ign, {
      ...
      if (result.status === "linked") {
        await interaction.editReply({
          embeds: [buildInfoEmbed("✅ Account Linked!", `Your Discord account is now linked to **${result.account.minecraftUsername}** (verified via your Hypixel Discord social field).`)],
        });
        return;
      }
